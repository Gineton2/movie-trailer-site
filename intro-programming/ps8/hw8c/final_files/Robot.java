...
//For the draft, define the instance variables, then finish the constructor and supply the getLocation method.
...
public class Robot
{
    ...
    public Robot(int theX, int theY)
    {
        ...
        pointLocation = new Point(theX,theY);
        startLocation = new Point(theX,theY);
        generator = new Random();
        generator.setSeed(12345);  //do not change this statement
    }
    
    /**
     * Returns a Point containing current x and y location of the Robot.
     * @return point containing current x,y location of robot
     */
    public Point getLocation()
    {
    ...
    //  3 is towards the left side of the window (west)
    /**
     * Moves the Robot by one unit in a random direction.
     */
    public void makeRandomMove()
    {
        ...
        {
            pointLocation.translate(1,0);
        }
        else if (direction == 3)
        {
            pointLocation.translate(-1,0);
        }
        //System.out.println("Moved to: " + pointLocation);
    }
    
    /**
     * Returns the distance the Robot is from the starting point.
     * @return distance the Robot is from starting point
     */
    public double getDistanceFromStart()
    {
        return startLocation.distance(pointLocation);
    }
}
