...
public class PictureUtil
{
    ...
    {
        int[][] flipped = pic.getGrayLevels();
        int temp;
        for (int i = 0; i < flipped.length; i++)
        {
            for (int j = 0; j < flipped[0].length/2; j++)
            {
                temp = flipped[i][j];
                flipped[i][j] = flipped[i][flipped[0].length-1-j];
                flipped[i][flipped[0].length-1-j] = temp;
            }
        }
    ...
    public static Picture grayAndRotate90( Picture pic)
    {
        ...
        for (int i = 0; i < gray.length; i++)
        {
            ...
            {
                rotated[j][i] = gray[gray.length-1-i][j];
            }
        }
        return new Picture(rotated);
    }
}
