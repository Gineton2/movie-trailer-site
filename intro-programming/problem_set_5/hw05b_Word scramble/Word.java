...
public class Word
{
   ...
   public String scramble(String word)
   {
        ...
        {
            i = generator.nextInt(length-1);
            j = i + 1 + generator.nextInt(length-i-1);
            
            String first = scramble.substring(0,i);
            String middle = scramble.substring(i+1,j);
            String last = scramble.substring(j+1);
            scramble = (first + scramble.substring(j, j + 1) + middle
                    + scramble.substring(i, i + 1) + last);
        }
